...
function renderNotificationEmail({
  ...
  const safeUrl = hasButton ? escapeHtml(buttonUrl) : "";
  ...
  const noteSection = note ? `
        <tr><td style="padding:0 32px 28px;font-family:Arial,sans-serif;">
          <p style="margin:0;padding:14px 16px;border-left:3px solid ${COLORS.green};background-color:#F8FAF8;font-size:14px;line-height:22px;color:${COLORS.text};">${escapeHtml(note)}</p>
        </td></tr>` : "";
...
<body style="margin:0;padding:0;background-color:${COLORS.cream};">
  <div style="display:none;font-size:1px;line-height:1px;color:${COLORS.cream};max-height:0;max-width:0;opacity:0;overflow:hidden;">${escapeHtml(preview || title)}</div>
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background-color:${COLORS.cream};"><tr><td align="center" style="padding:28px 12px 40px;">
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="width:100%;max-width:600px;border:1px solid ${COLORS.border};border-radius:16px;background-color:#FFFFFF;">
      <tr><td style="padding:24px 32px;background-color:${COLORS.teal};border-radius:15px 15px 0 0;"><table role="presentation" cellpadding="0" cellspacing="0" width="100%"><tr>
        <td width="46" valign="middle"><div style="width:42px;height:42px;border-radius:10px;background-color:${COLORS.green};text-align:center;line-height:42px;font-family:Arial,sans-serif;font-size:17px;font-weight:800;color:${COLORS.teal};">MN</div></td>
        <td valign="middle" style="padding-left:12px;font-family:Arial,sans-serif;font-size:17px;line-height:23px;font-weight:700;color:#FFFFFF;">Mahmoud Nagy's Team<br><span style="font-size:12px;font-weight:400;letter-spacing:1px;color:#D9E8E2;">STUDENT PLATFORM</span></td>
      </tr></table></td></tr>
      <tr><td style="padding:32px 32px 24px;font-family:Arial,sans-serif;">
        <p style="margin:0 0 10px;font-size:12px;line-height:18px;letter-spacing:1.5px;font-weight:700;color:${COLORS.teal};">${escapeHtml(eyebrow || "Platform notification").toUpperCase()}</p>
        <h1 style="margin:0 0 20px;font-size:28px;line-height:35px;font-weight:700;color:${COLORS.teal};">${escapeHtml(title)}</h1>
        <p style="margin:0 0 14px;font-size:16px;line-height:25px;color:${COLORS.text};">Hello ${escapeHtml(name || "there")},</p>
        <p style="margin:0;font-size:16px;line-height:25px;color:${COLORS.text};">${escapeHtml(message)}</p>
      </td></tr>
      ${buttonSection}${noteSection}
      <tr><td style="padding:20px 32px;border-top:1px solid ${COLORS.border};font-family:Arial,sans-serif;"><p style="margin:0;font-size:13px;line-height:21px;color:${COLORS.muted};">This is an automated message from Mahmoud Nagy's Team.</p></td></tr>
    </table>
  </td></tr></table>
</body></html>`;
}

module.exports = { renderEmail, renderNotificationEmail, escapeHtml };
