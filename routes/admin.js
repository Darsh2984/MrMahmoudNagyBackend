...
router.get("/export-users", async (req, res) => {
  try {
    ...
    users.forEach((u) => {
      ...
      if (u.role === "teacher") phone = u.teacherPhone || "N/A";
    ...
    worksheet.getRow(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF0B3C49" } };
  ...
  try {
    ...
    students.forEach((s) => {
      worksheet.addRow({
        name: s.name,
        email: s.email,
        phone: s.studentPhone || "N/A",
        school: s.schoolId?.name || "N/A",
        parentName: s.parentId?.name || s.parentName || "N/A",
        parentEmail: s.parentId?.email || "N/A",
        parentPhone: s.parentId?.parentPhone || s.parentPhone || "N/A",
      });
    });
